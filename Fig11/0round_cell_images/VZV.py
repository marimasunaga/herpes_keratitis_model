...
gamma_values = np.arange(1.0, 2.01, 0.1) #Cytokine secretion rate
...
seed_values = list(range(5))  # Different random seeds for multiple runs
...
output_dir = os.path.join(script_dir, "modified_model")
...
for gamma in gamma_values:
    for seed in seed_values:
        ...
        # Run simulation
        u, v, t = uv_simulation(alpha, beta, gamma, delta, dv, dw, C, n, seed)
        # Save figure with descriptive filename
        filename = os.path.join(
            output_dir,
            f"plot_a{alpha:.3f}_b{beta:.3f}_g{gamma:.3f}_"
            f"d{delta:.3f}_Dv{dv:.3f}_DW{dw}_C{C:.3f}_iter{t}_seed{seed}.png"
        )
