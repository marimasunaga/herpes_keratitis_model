...
def uv_simulation(alpha, beta, gamma, delta, dv):
    ...
    where (Type B): S_{i,j}(t) = u_{i,j}(t) + B_{i,j}(t).                              (Eq. S_B)
    ...
    for t in range(loopnumber):
        ...
        u = u + h
        # Cytokine source term (Type B): S_{i,j}(t) = u_{i,j}(t) + B_{i,j}(t)
        S = u + b
...
gamma = 0.5 #Cytokine secretion rate
...
# ==============================================================
# Run simulation
u, v, t = uv_simulation(alpha, beta, gamma, delta, dv)
...
filename_u = os.path.join(
    script_dir,
    f"TypeB_u_plot_a{alpha:.3f}_b{beta:.3f}_g{gamma:.3f}_"
...
filename_v = os.path.join(
    script_dir,
    f"TypeB_v_plot_a{alpha:.3f}_b{beta:.3f}_g{gamma:.3f}_"
    f"d{delta:.3f}_Dv{dv:.3f}_iter{t}.png"
)
plt.imsave(
    filename_v,
