...
def uv_simulation(alpha, beta, gamma, delta, dv, n):
   ...
   Simulate the time evolution of u_{i,j}(t) and v_{i,j}(t)
    ...
    where (Type D): S_{i,j}(t) = B^n_{i,j}(t).                                         (Eq. S_D)
    ...
    for t in range(loopnumber):
        ...
        u = u + h
        # Cytokine source term (Type D): S_{i,j}(t) = B^n_{i,j}(t)
        S = bn
...
gamma = 3.6 #Cytokine secretion rate
delta = 0.5 #Cytokine degradation rate
n = 1 #The number of layers producing cytokine

# ==============================================================
# Main parameter loop
# ==============================================================
# Run simulation
u, v, t = uv_simulation(alpha, beta, gamma, delta, dv, n)
import os
# Get the absolute path of the directory where this script is located
script_dir = os.path.dirname(os.path.abspath(__file__))

filename_u = os.path.join(
    script_dir,
    f"TypeD_u_plot_a{alpha:.3f}_b{beta:.3f}_g{gamma:.3f}_"
    f"d{delta:.3f}_Dv{dv:.3f}_iter{t}.png"
)

# Save dot representation of u
save_dot_plot(u, filename_u)

filename_v = os.path.join(
    script_dir,
    f"TypeD_v_plot_a{alpha:.3f}_b{beta:.3f}_g{gamma:.3f}_"
    f"d{delta:.3f}_Dv{dv:.3f}_iter{t}.png"
)
plt.imsave(
    filename_v,
