...
def uv_simulation(alpha, beta, gamma, delta, dv):
    ...
    where (Type C): S_{i,j}(t) = B_{i,j}(t).                                           (Eq. S_C)
    ...
    for t in range(loopnumber):
        ...
        u = u + h
        # Cytokine source term (Type C): S_{i,j}(t) = B_{i,j}(t)
        S = b
...
gamma = 5.0 #Cytokine secretion rate
...
filename_u = os.path.join(
    script_dir,
    f"TypeC_u_plot_a{alpha:.3f}_b{beta:.3f}_g{gamma:.3f}_"
...
filename_v = os.path.join(
    script_dir,
    f"TypeC_v_plot_a{alpha:.3f}_b{beta:.3f}_g{gamma:.3f}_"
    f"d{delta:.3f}_Dv{dv:.3f}_iter{t}.png"
)
plt.imsave(
    filename_v,
